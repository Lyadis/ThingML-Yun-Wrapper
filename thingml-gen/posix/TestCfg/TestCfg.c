...
struct LinuxSerial2_Instance TestCfg_sp_var;
...
if (_instance == &TestCfg_bridge_var) {
LinuxSerial2_handle_serial_serial_tx(&TestCfg_sp_var, b);
}
}
...
if (_instance == &TestCfg_bridge_var) {
LinuxSerial2_handle_serial_serial_open(&TestCfg_sp_var, device, baudrate);
}
}
...
TestCfg_sp_var.LinuxSerial2_LinuxSerialImpl_State = LINUXSERIAL2_LINUXSERIALIMPL_RUNNING_STATE;
...
TestCfg_bridge_var.Bridge_bps__var = 9600;

Bridge_BridgeChart_OnEntry(BRIDGE_BRIDGECHART_STATE, &TestCfg_bridge_var);
LinuxSerial2_LinuxSerialImpl_OnEntry(LINUXSERIAL2_LINUXSERIALIMPL_STATE, &TestCfg_sp_var);
